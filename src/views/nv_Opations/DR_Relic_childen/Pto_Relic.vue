...
<!--   <label for="imgUpload" class='dragImg' >将图片文件拖拽至此，或 <span class='colorRed'>点击选择文件上传</span>
    ...
    <div id='previewImg'  @click="deleteImg($event)"></div>
    <div class='buttons'>
    	<el-button class='startUpload' id='imgSubmit' @click='onSubmit'>开始上传</el-button>
    	<el-button class='stopUpload'>停止上传</el-button> 
    </div>
...
import { uploadPostImg } from '../../../api/api'
import qs from 'qs'
	export default {
		components: {
			Dropzone
				...
				for (var i = 0; i < fileA.length; i++) {
					...
					reader.onload = (function(file, i){
						return function(e){
							...
							var imgstr = '<div id="preview'+ _this.count +'" data-id="'+ _this.count +'" class="previewPicCont" ><img data-id='+ _this.count +' class="previewPic" src="'+e.target.result+'" /><div class="imgDelete"><span>'+file.name+'</span><span class="deleteIcon"></span></div></div>';
        ...
        mounted() {
            ...
            this.$emit('inSetting'); // 进入“导入文物图片”页面后改变左侧菜单显示
        },
        beforeDestroy() {
            this.$emit('outSetting'); // 离开“导入文物图片”页面后改变左侧菜单显示
        },
		deactivated(){
			this.$destroy(true);
		}
...
.imgDelete .deleteIcon{
	...
	float: right;
	cursor: pointer;
	background: url(../../../../static/img/delete_white.png) no-repeat 9px 7px;
}
.imgDelete .deleteIcon:hover{
	background: url(../../../../static/img/delete_blue.png) no-repeat 9px 7px;
}
...
.buttons .startUpload:focus, .buttons .startUpload:hover{
	color: #666;
}
.buttons .stopUpload{
	height: 32px;
