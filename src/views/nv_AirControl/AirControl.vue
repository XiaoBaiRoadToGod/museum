<template>
	...
	<div class='listContainer' v-loading='loading'>
		<div class='myAir' v-for='(item, index) in dataList' :key='index'>
			<div class='airContainer' v-if='item.VER_ID == 17'>
				...
				<div class='airContent type0Background'>
					<div class="airValue positonAb width16">
						新风阀开度
						<i>{{ item.airValueOpening}}</i>%
					</div>
					<div class="returnAir positonAb">
						<span>回风温度：
							<i>{{ item.returnTemp }}</i>℃</span>
						<span>回风湿度：
							<i>{{ item.returnHumi}} </i>%</span>
					</div>
					<div class="pressAlerm positonAb width16">
						压差报警
						<i class='airIcon pressAlermIcon' :style='{background: item.differentialIcon }'></i>
					</div>
					...
					<div class="airSupply positonAb width16">
						送风机段
						<i class='airIcon' :style='{background: item.blower }'></i>
					</div>
					<div class="returnAirSeg positonAb width16">
						回风机段
						<i class='airIcon' :style='{background: item.return }'></i>
					</div>
					<div class="pressureSeg positonAb width16">
						压差报警
						<i class='airIcon' :style='{background: item.differentialIcon }'></i>
					</div>
					...
					<div class="alermBtn positonAb">
						...
						<div class='switch'>
							开关机：
							<el-switch v-model="item.switch" on-color="#438eb9" off-color="#a63830" size='mini' :width='50' @change='switchChange($event, item.loggersn, index)'>
							</el-switch>
						</div>
					</div>
				</div>
			...
			<div class='airContainer typeContainer' v-if='item.VER_ID == 18'>
				<div class='airTitle'>
					<span class='type'>调控模式：{{item.Freshair}}</span>
				...
				<div class='airContent type1Background'>
					...
					<div class='clickSetContainer positonAb'>
						<div class='backFanTemp' @click='setValue($event, item.loggersn, index, "回风机温度")'>
							<span class='name'>回风机温度设定：{{item.Temp}}</span>℃
						</div>
		...
		<el-dialog class='airAlermDialog' :visible.sync="alermInfoDialog" width='800px' v-if='airInfo !== []' title='故障信息'>
			<div class='infoContainer'>
				<div class='loggerAlermInfoContainer'>
					<span class='loggerName'>送风机过载:</span>
					<i class='alermIcon' :style='{background: airInfo.differential == 1 ? airInfoAlerm : airInfoNormal}'></i>
				</div>
				<div class='loggerAlermInfoContainer'>
					<span class='loggerName'>压缩机1过载:</span>
					<i class='alermIcon' :style='{background: airInfo.State1 == 1 ? airInfoAlerm : airInfoNormal}'></i>
				</div>
				<div class='loggerAlermInfoContainer'>
					<span class='loggerName'>送风机控制:</span>
					<i class='alermIcon' :style='{background: airInfo.State4 == 0 ? airInfoAlerm : airInfoNormal}'></i>
				</div>
				<div class='loggerAlermInfoContainer'>
					<span class='loggerName'>送风风压保护:</span>
					<i class='alermIcon' :style='{background: airInfo.cooling == 1 ? airInfoAlerm : airInfoNormal}'></i>
				</div>
				<div class='loggerAlermInfoContainer'>
					<span class='loggerName'>冷凝风机1过载:</span>
					<i class='alermIcon' :style='{background: airInfo.State2 == 1 ? airInfoAlerm : airInfoNormal}'></i>
				</div>
				<div class='loggerAlermInfoContainer'>
					<span class='loggerName'>压缩机1控制:</span>
					<i class='alermIcon' :style='{background: airInfo.State5 == 0 ? airInfoAlerm : airInfoNormal}'></i>
				</div>
				<div class='loggerAlermInfoContainer'>
					<span class='loggerName'>火灾信号连锁保护:</span>
					<i class='alermIcon' :style='{background: airInfo.thermal == 1 ? airInfoAlerm : airInfoNormal}'></i>
				</div>
				<div class='loggerAlermInfoContainer'>
					<span class='loggerName'>压缩机1高压保护:</span>
					<i class='alermIcon' :style='{background: airInfo.compress == 1 ? airInfoAlerm : airInfoNormal}'></i>
				</div>
				<div class='loggerAlermInfoContainer'>
					<span class='loggerName'>压缩机2控制:</span>
					<i class='alermIcon' :style='{background: airInfo.State6 == 0 ? airInfoAlerm : airInfoNormal}'></i>
				</div>
				<div class='loggerAlermInfoContainer'>
					<span class='loggerName'>电加热过热保护:</span>
					<i class='alermIcon' :style='{background: airInfo.hcation == 1 ? airInfoAlerm : airInfoNormal}'></i>
				</div>
				<div class='loggerAlermInfoContainer'>
					<span class='loggerName'>压缩机1低压保护:</span>
					<i class='alermIcon' :style='{background: airInfo.mainBlower == 1 ? airInfoAlerm : airInfoNormal}'></i>
				</div>
				<div class='loggerAlermInfoContainer'>
					<span class='loggerName'>系统2能量阀控制:</span>
					<i class='alermIcon' :style='{background: airInfo.State7 == 0 ? airInfoAlerm : airInfoNormal}'></i>
				</div>
				<div class='loggerAlermInfoContainer'>
					<span class='loggerName'>加湿器故障:</span>
					<i class='alermIcon' :style='{background: airInfo.blower == 1 ? airInfoAlerm : airInfoNormal}'></i>
				</div>
				<div class='loggerAlermInfoContainer'>
					<span class='loggerName'>压缩机2过载:</span>
					<i class='alermIcon' :style='{background: airInfo.HWater == 1 ? airInfoAlerm : airInfoNormal}'></i>
				</div>
				<div class='loggerAlermInfoContainer'>
					<span class='loggerName'>电加热1控制:</span>
					<i class='alermIcon' :style='{background: airInfo.State8 == 0 ? airInfoAlerm : airInfoNormal}'></i>
				</div>
				<div class='loggerAlermInfoContainer'>
					<span class='loggerName'>水泵及水流量连锁输入:</span>
					<i class='alermIcon' :style='{background: airInfo.return == 1 ? airInfoAlerm : airInfoNormal}'></i>
				</div>
				<div class='loggerAlermInfoContainer'>
					<span class='loggerName'>冷凝风机2过载:</span>
					<i class='alermIcon' :style='{background: airInfo.freshPolice == 1 ? airInfoAlerm : airInfoNormal}'></i>
				</div>
				<div class='loggerAlermInfoContainer'>
					<span class='loggerName'>电加热2控制:</span>
					<i class='alermIcon' :style='{background: airInfo.State9 == 0 ? airInfoAlerm : airInfoNormal}'></i>
				</div>
				<div class='loggerAlermInfoContainer'>
					<span class='loggerName'>送风机过滤压差开关:</span>
					<i class='alermIcon' :style='{background: airInfo.fresh == 1 ? airInfoAlerm : airInfoNormal}'></i>
				</div>
				<div class='loggerAlermInfoContainer'>
					<span class='loggerName'>压缩机2高压保护:</span>
					<i class='alermIcon' :style='{background: airInfo.DiffPolice == 1 ? airInfoAlerm : airInfoNormal}'></i>
				</div>
				<div class='loggerAlermInfoContainer'>
					<span class='loggerName'>加湿器控制:</span>
					<i class='alermIcon' :style='{background: airInfo.State10 == 0 ? airInfoAlerm : airInfoNormal}'></i>
				</div>
				<div class='loggerAlermInfoContainer'>
					<span class='loggerName'>压缩机2低压保护:</span>
					<i class='alermIcon' :style='{background: airInfo.State3 == 1 ? airInfoAlerm : airInfoNormal}'></i>
				</div>
			</div>
...
<script>
import { GetAirControlDetails, ControlsSet } from "../../api/api";
export default {
  data() {
    return {
      loading: false,
      ...
      GetAirControlDetails().then(res => {
        ...
        for (let idx = 0; idx < res.length; idx++) {
          this.dataList.push({
            ...
            differential:
              res[idx].VER_ID == 17
                ? res[idx].Differential == "正常" ? "#438eb9" : "#a63830"
                : res[idx].Differential, // 压差报警   18送风机过载
            differentialIcon:
              res[idx].Differential == "正常" ? this.airNomal : this.airAlerm,
            ...
            blower:
              res[idx].VER_ID == 17
                ? res[idx].Blower == "正常" ? this.airNomal : this.airAlerm
                : res[idx].Blower, // 送风机段  18 加湿器故障
            return:
              res[idx].VER_ID == 17
                ? res[idx].Return == "正常" ? this.airNomal : this.airAlerm
                : res[idx].Return, // 回风机段  18 水泵及水流量
            ...
            compress:
              res[idx].VER_ID == 17
                ? res[idx].CompressO == "正常" ? "#438eb9" : "#a63830"
                : res[idx].CompressO, // 送风阀报警   18 压缩机1高压保护
            mainBlower:
              res[idx].VER_ID == 17
                ? res[idx].MainBlowerO == "正常" ? "#438eb9" : "#a63830"
                : res[idx].MainBlowerO, // 回风阀报警   18 压缩机1低压保护
            HWater:
              res[idx].VER_ID == 17
                ? res[idx].HWater == "正常" ? "#438eb9" : "#a63830"
                : res[idx].HWater, // 风机故障报警   18 压缩机2 过载
            freshPolice:
              res[idx].VER_ID == 17
                ? res[idx].FreshPolice == "正常" ? "#438eb9" : "#a63830"
                : res[idx].FreshPolice, // 新风阀报警 18 冷凝风2过载
